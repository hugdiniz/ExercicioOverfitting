...
type Legendre
  ...
  function Legendre(qf,sigma)
    ...
    this.qf = qf
    this.sigma = sigma
    this.poly = [pol for pol in sp.legendre(qf, monic=true)]
    ...
    this.generateXPoint = function ()
      return rand(Bool) ? rand() : -rand()
    end
    this.generateYPoint = function (x)
      return sum([this.poly[i]*(x^(i-1)) for i = 1:length(this.poly)]) + ( rand(Bool)? rand(0.0:0.001:this.sigma) : -rand(0.0:0.001:this.sigma))
    end
